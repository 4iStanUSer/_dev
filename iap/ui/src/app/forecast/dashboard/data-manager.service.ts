...
export class DataManagerService {
    ...
    getObs() {
        if (!this.isFilled) {
            this.isFilled = new Observable((observer) => {
    ...
    getDataModel() {
        ...
        resp.subscribe((d)=> {
            ...
            this.periods = {
                main: d['main_period'],
                decomposition: d['decomp_period']
            };

            // Transformation of insights
            this.insights = d['insights'].map((insight) => {
                return {
                    text: insight
                };
            });
    ...
    getData_ForecastAbsValues(timescale: string, timepoints: Array<string>, variable: string) {
        ...
        for (let j = 0; j < pointsValue.length; j++) {
            ...
            if (j != 0) { // Change this use or growth or cagrs
                output['rate'].push(pointsValue[j].gr);
            }
            // !!!!!!
        }
        return output;
    ...
    getData_ForecastRateValues(timescale: string, timepoints: Array<string>, variable: string) {
    ...
    getData_Decomposition(type: string, timescale: string,
        ...
        let l = items.length;
        // let v = this.dataModel.getVariable(type);
        for (let i = 0; i < l; i++) {
            output['abs'].push({
                name: items[i]['name'],
                value: items[i]['value'],
    ...
    getDecompPeriodLabels() {
        let timescales = this.config['decomp_timescales'];
