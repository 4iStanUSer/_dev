...
class Storage(IStorage):
    ...
    def _save_data_frame(self, project_name = None, entity_path = None, var_name = None, time_scale_name = None,
                         time_series=None, values = None):

        serie = pd.DataFrame(data = dict(Project = project_name, Entity=entity_path, Variable = var_name,
                                         TimeScale = time_scale_name, TimeSeries = time_series, Value = values))

        self.data_frame.append(serie)

    def _get_data_frame(self):
        return self.data_frame

    def save_to_local_storage(self, project_name):
        self.data_frame.save_to_csv("{0}.csv".forma(project_name))
